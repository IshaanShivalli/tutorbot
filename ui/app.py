...
"""
    ...
    def handle_login (self ,argument :str ):
        ...
        try :
            ...
            fallback =""
            if not sent :
                fallback =f" If email delivery is not configured, use code [bold]{verification['code']}[/bold] directly."
            self .add_message (
            "system",
            f"Password verified. A confirmation code has been sent to [bold]{user['email']}[/bold]. "
            f"Enter [bold]/verify {username} <code>[/bold] to complete login.{fallback}",
            )
    ...
    def handle_register (self ,argument :str ):
        ...
        try :
            ...
            self .add_message (
            "system",
            f"Account created for [bold]{user['username']}[/bold]. A confirmation code was sent to [bold]{user['email']}[/bold]. "
            f"Enter [bold]/verify {username} <code>[/bold] to complete registration.{fallback}",
            )
        except KeyError as e :
            self .add_message ("error",str (e ))
        except ValueError as e :
