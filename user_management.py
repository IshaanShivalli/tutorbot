...
def _load_data() -> dict[str, Any]:
    ...
    for user in raw["users"].values():
        user.setdefault("weak_subject", None)
    return raw
...
def set_user_weak_subject(username: str, subject: str) -> dict[str, Any]:
    username = username.strip().lower()
    if not username:
        raise ValueError("Username cannot be empty")
    ...
    if not user:
        raise KeyError(f"User '{username}' does not exist")
    user["weak_subject"] = subject.strip().lower()
    _save_data(data)
    return user
