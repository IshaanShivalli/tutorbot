...
Parses structured quiz content (as produced by DOC_SYSTEM_PROMPT / generate_quiz_for_doc)
and generates a Word (.docx) file — one version with questions only, and optionally an
answer key section or a separate answer-key document.
"""
...
def parse_quiz_text(quiz_text):
    """
    Parses the structured plain-text quiz format into a Python data structure.
    ...
    # Split into blocks starting at each "Q<n>:"
    blocks = re.split(r"\n(?=Q\d+:)", quiz_text)
    ...
    for block in blocks:
        ...
        # More flexible answer matching - capture everything after ANSWER: until next field or end
        answer_match = re.search(r"ANSWER:\s*(.+?)(?=EXPLANATION:|$)", block, re.DOTALL)
        
        # More flexible explanation matching - capture everything after EXPLANATION: until next Q or end
        explanation_match = re.search(r"EXPLANATION:\s*(.+?)(?=Q\d+:|$)", block, re.DOTALL)

        options = re.findall(r"^([A-Z]\))\s*(.+)$", block, re.MULTILINE)

        # Clean up answer and explanation text
        answer_text = answer_match.group(1).strip() if answer_match else ""
        explanation_text = explanation_match.group(1).strip() if explanation_match else ""
        
        # Remove trailing markdown/formatting characters
        ...
        data["questions"].append({
            ...
            "options": options,  # list of (label, text) tuples
            "answer": answer_text,
...
def create_quiz_document(quiz_text, output_path="quiz.docx", include_answers=False):
    """
    Creates a .docx quiz document from structured quiz text.

    Args:
        quiz_text: structured quiz string (from generate_quiz_for_doc)
        output_path: file path to save the .docx to
        include_answers: if True, appends an "Answer Key" section at the end;
                          if False, produces a clean student-facing quiz only
    """
    ...
    for q in data["questions"]:
        ...
        if q["options"]:
            ...
            pass  # blank is already embedded in the question text
    ...
    # --- Answer key (optional, appended to same document) ---
    if include_answers:
        doc.add_page_break()
...
def create_quiz_and_answer_key(quiz_text, quiz_path="quiz.docx", answer_key_path="answer_key.docx"):
    """
    Convenience function: generates two separate documents —
    a clean student-facing quiz, and a separate answer key document.
    """
    create_quiz_document(quiz_text, output_path=quiz_path, include_answers=False)

    # Build a standalone answer-key doc
    data = parse_quiz_text(quiz_text)
    doc = Document()
